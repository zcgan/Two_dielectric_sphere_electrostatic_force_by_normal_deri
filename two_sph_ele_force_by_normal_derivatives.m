...
%%%%%%%%%%about input files%%%%%%%%%%%%%%%%%%%%%%%%
%All we need is the location of each collocation point, the normal derivative on each collocation point, and
...
src_loc=sourcedata(:,2:4); %location of the central charges
src_char=sourcedata(:,5); %magnitude of the central charges
eps_i=sourcedata(1,6); %now we assume each sphere has the same dielectric constant inside
eps_s=sourcedata(1,7);
N=sourcedata(:,8); %the number of triangles on each sphere

index=zeros(2,2); %record the start and end index for each sphere
index(1,1)=1; index(1,2)=N(1);
index(2,1)=N(1)+1; index(2,2)=N(1)+N(2);
